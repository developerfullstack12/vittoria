		...
		$output = "
			<?php
				...
				/*
					...
					*/
				add_action(  'wp_enqueue_scripts', '{$slug_no_hyphens}_child_enqueue_styles', 60 );
				 ...
				 * 1. Copy file '/wp-content/themes/{$slug}/js/components.js' to '/wp-content/themes/{$slug}-child/js/components.js'
				 * 2. Uncomment the code lines below
				*/

				// add_action( 'wp_enqueue_scripts', '{$slug_no_hyphens}_child_enqueue_custom_scripts', 60 );
				// function {$slug_no_hyphens}_child_enqueue_custom_scripts() {
				//   // unload parent theme frontend
				//   wp_deregister_script( '{$slug}-components' );
				//   wp_dequeue_script( '{$slug}-components' );

				//   // load child theme custom frontend from /{$slug}-child/js/components.js
				//   wp_enqueue_script( '{$slug}-components', get_stylesheet_directory_uri() . '/js/components.js', array( 'modernizr', 'jquery', 'barba', 'isotope', 'imagesloaded' ), wp_get_theme()->get( 'Version' ), true );
				// }\n
		";
